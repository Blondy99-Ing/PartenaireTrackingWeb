...
class GpsControlService
{
    ...
    private function getWithParams(string $url, array $params): array
    {
        try {
            ...
            return $this->decodeHttpResponse($res, $url);
        } catch (\Throwable $e) {
    ...
    /**
     * Décode une réponse provider déjà reçue (ASMX -> tableau normalisé).
     * Extrait de getWithParams() pour être réutilisable par les appels
     * poolés (Http::pool) où la requête est envoyée séparément.
     *
     * @param \Illuminate\Http\Client\Response|\Throwable $res
     */
    private function decodeHttpResponse($res, string $url = ''): array
    {
        if ($res instanceof \Throwable) {
            Log::error('[GPS] Exception HTTP (pool)', [
                'account' => $this->account,
                'url' => $url,
            ...
            return [
                'success' => 'false',
                'errorCode' => '500',
                'errorDescribe' => 'Exception HTTP: ' . $res->getMessage(),
            ];
        }
    ...
    /**
     * Post-traitement commun à getLatestLocationByUserId() : ne garde que le
     * 1er enregistrement utile d'une réponse provider déjà décodée.
     */
    private function extractSingleLatestRecordFromDecoded(array $resp): ?array
    {
    ...
    public function getEngineStatusFromLastLocation(string $macId): array
{
    ...
    public function getLatestLocationByUserId(string $userId, ?string $mapType = null, ?string $option = null): ?array
    {
        ...
        return $this->extractSingleLatestRecordFromDecoded($resp);
    }

    /**
     * Variante concurrente de getLatestLocationByUserId() : envoie tous les
     * appels provider EN PARALLÈLE (Http::pool / curl_multi) au lieu d'un
     * par un. Un même appel séquentiel prend jusqu'à httpTimeoutSeconds
     * (20 s par défaut) PAR boîtier ; sur ~250 boîtiers cela rendrait tout
     * sync périodique de toute la flotte impraticable (>1h dans le pire
     * cas). En pool, le temps total d'un lot est borné par le plus lent des
     * appels du lot, pas par leur somme.
     *
     * Nécessite un compte déjà sélectionné (setAccount) : tous les user_id
     * du lot doivent appartenir au même compte GPS (même token).
     *
     * @param array<string,string> $userIdsByKey [clé arbitraire (ex: mac_id) => objectid/user_id]
     * @return array<string,array|null>           [clé => enregistrement provider ou null]
     */
    public function getLatestLocationsBatchPooled(array $userIdsByKey, ?string $mapType = null, ?string $option = null): array
    {
        $userIdsByKey = array_filter(
            $userIdsByKey,
            fn ($uid) => trim((string) $uid) !== ''
        );

        if (empty($userIdsByKey)) {
            return [];
        }

        $token = $this->loginGps(false);
        if (!$token) {
            return array_fill_keys(array_keys($userIdsByKey), null);
        }
        ...
        $responses = Http::pool(function (\Illuminate\Http\Client\Pool $pool) use ($userIdsByKey, $mapType, $option, $token, $url) {
            $requests = [];
            foreach ($userIdsByKey as $key => $userId) {
                $requests[$key] = $pool->as($key)->timeout($this->httpTimeoutSeconds)->get($url, [
        ...
        $out = [];
        foreach (array_keys($userIdsByKey) as $key) {
            $decoded = $this->decodeHttpResponse($responses[$key] ?? new \RuntimeException('Pas de réponse (pool)'), $url);
            $out[$key] = $this->extractSingleLatestRecordFromDecoded($decoded);
        }
...
/**
 * ✅ BATCH CONCURRENT : même but que syncAndSaveLatestLocationsFromAllSimGps()
 * (rafraîchir `locations` pour toute la flotte depuis 18gps) mais en
 * interrogeant le provider par lots PARALLÈLES (Http::pool) au lieu d'un
 * device à la fois. La version séquentielle n'a jamais pu être planifiée
 * en production car un passage complet dépasserait largement l'intervalle
 * entre deux exécutions (jusqu'à 20 s x ~250 boîtiers = plusieurs dizaines
 * de minutes). Celle-ci reste de l'ordre de quelques dizaines de secondes,
 * ce qui la rend utilisable toutes les 1-2 minutes via le scheduler
 * (voir gps:sync-engine-status) : la page de coupure manuelle continue de
 * lire uniquement `locations` (rapide, aucun appel provider synchrone dans
 * la requête web) mais cette table reste alignée sur l'état réel 18gps à
 * quelques minutes près au lieu de dépendre uniquement du prochain
 * heartbeat spontané du boîtier.
 */
public function syncEngineStatusFleetConcurrent(int $poolSize = 20, bool $forceRefreshLogin = false): array
{
    ...
    foreach ($groups as $account => $items) {
        ...
        foreach ($items->chunk($poolSize) as $chunk) {
            $userIdsByMac = [];
            foreach ($chunk as $sim) {
                $userIdsByMac[trim((string) $sim->mac_id)] = trim((string) $sim->objectid);
            }

            try {
                $records = $this->getLatestLocationsBatchPooled($userIdsByMac);
            } catch (\Throwable $e) {
                $summary['failed'] += count($userIdsByMac);
                $summary['by_account'][$account]['failed'] += count($userIdsByMac);
                $summary['failures'][] = ['account' => $account, 'reason' => 'pool exception: ' . $e->getMessage(), 'count' => count($userIdsByMac)];
                continue;
            }

            foreach ($userIdsByMac as $macId => $userId) {
                $record = $records[$macId] ?? null;

                if (!$record || !is_array($record)) {
                    $summary['failed']++;
                    $summary['by_account'][$account]['failed']++;
                    $summary['failures'][] = ['mac_id' => $macId, 'account' => $account, 'user_id' => $userId, 'reason' => 'no provider record'];
                    continue;
                }

                $save = $this->saveLatestLocationRecordToDbNormalized($record, $macId, true);
                ...
                if (($save['success'] ?? false) && ($save['saved'] ?? false)) {
                    ...
                    $summary['failed']++;
                    $summary['by_account'][$account]['failed']++;
                    $summary['failures'][] = ['mac_id' => $macId, 'account' => $account, 'user_id' => $userId, 'reason' => $save['reason'] ?? 'save failed'];
                }
            }
        }
    }
