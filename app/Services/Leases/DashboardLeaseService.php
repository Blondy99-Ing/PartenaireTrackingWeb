...
class DashboardLeaseService
{
    ...
    private function buildCutoffSummary(array $cutoffData): array
    {
        ...
        return [
            ...
            'reactivation_pending' => $histories->where('status', 'REACTIVATION_REQUESTED_AFTER_FORGIVENESS')->count(),
            'reactivated' => $histories->where('status', 'REACTIVATED_AFTER_FORGIVENESS')->count(),
            'reactivation_failed' => $histories->where('status', 'REACTIVATION_FAILED_AFTER_FORGIVENESS')->count(),
    ...
    private function shortCutoffReason(LeaseCutoffHistory $history): string
    {
        ...
        return match ($status) {
            ...
            'CANCELLED_DAY_EXPIRED' => 'Échéance expirée sans coupure confirmée : aucune coupure rétroactive, jour suivant traité indépendamment.',
    ...
    private function cutoffStatusLabel(string $status): string
    {
        return match ($status) {
            ...
            'CANCELLED_DAY_EXPIRED' => 'Échéance expirée',
            'FAILED' => 'Échec',
            default => $status ?: '—',
        };
    }
    ...
    private function cutoffBadge(string $status): string
    {
        return match ($status) {
            ...
            'COMMAND_SENT' => 'info',
            'CUT_OFF' => 'success',
            'CANCELLED_PAID', 'CANCELLED', 'CANCELLED_FORGIVEN_BEFORE_CUT', 'CANCELLED_DAY_EXPIRED' => 'muted',
